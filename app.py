...
def after_request(response):
    response.headers.add('Access-Control-Allow-Origin', 'https://teamlava-bah-25-ps5.vercel.app')
...
def index():
    return jsonify({
        ...
        "status": "running",
        "version": "1.0.0",
        "endpoints": ["/health", "/upload", "/static/output/<filename>"]
    })
...
def health_check():
    ...
    try:
        ...
        # Overall health status (temporarily allow running without models for testing)
        is_healthy = dirs_exist  # Only check directories for now
        ...
        # Return 200 even if models aren't loaded - server is still functional
        status_code = 200
